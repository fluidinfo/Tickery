...
class MaxRequestsLimit(HorizontalPanel):
    def __init__(self):
        HorizontalPanel.__init__(self, Spacing=4)
        self.add(Label('MaxRequests limit:', StyleName='section'))
        self.value = Label()
        self.add(self.value)
        ...
        self.update = Button('Update', MaxRequestsUpdate(self))
        self.add(self.update)
        displayer = MaxRequestsDisplay(self)
        self.refresh = Button('Refresh', displayer, StyleName='refresh')
        self.add(self.refresh)
        self.err = Label()
        self.add(self.err)
        remote = server.AdminService()
        id = remote.getMaxRequestsLimit(displayer)
        if id < 0:
            self.err.setText('oops: could not call getMaxRequestsLimit')


class MaxRequestsDisplay(object):
    ...
    def onClick(self, sender):
        ...
        remote = server.AdminService()
        id = remote.getMaxRequestsLimit(self)
        if id < 0:
            self.add(Label('oops: could not call getMaxRequestsLimit'))
    ...
    def onRemoteError(self, code, message, request_info):
        self.sender.refresh.setEnabled(True)
        self.sender.err.setText('Could not getMaxRequestsLimit: ' + message)


class MaxRequestsUpdate(object):
    def __init__(self, sender):
        self.sender = sender
    ...
    def onClick(self, sender):
        ...
        if value == '':
            ...
            remote = server.AdminService()
            id = remote.setMaxRequestsLimit(value, self)
            if id < 0:
                self.sender.err.setText('oops: could not setMaxRequestsLimit')
